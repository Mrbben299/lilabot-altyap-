...
exports.run = (client, message, args) => {
    ...
    var akardiyan = akardiyan[Math.floor(Math.random(1) * akardiyan.length)]
    const akardiyanembed = new Discord.MessageEmbed()
    .setColor("RANDOM")
    .setAuthor(`İlginç Bilgi`, message.author.avatarURL({ dynamic: true }))
    .setFooter(`${message.author.username}`)
    .setDescription(`> ${akardiyan}`)
    return message.channel.send(akardiyanembed);
};

exports.config = {
  name: 'ilginç-bilgi',
  aliases: ['ilginç-bilgi']
};
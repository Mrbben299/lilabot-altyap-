...
client.on("message", async message => {
    ...
    if (cmd === "aktivite") {
        ...
        if (!channel || channel.type !== "voice") return message.channel.send("⛔ | Lütfen Geçerli Bir `Sesli Kanal İD` Girin");
        if (!channel.permissionsFor(message.guild.me).has("CREATE_INSTANT_INVITE")) return message.channel.send("⛔ | `Davet Oluştur` İznine İhtiyacım Var");
      ...
      const formatlar = new Discord.MessageEmbed()
        ...
        fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
            ...
            .then(res => res.json())
            .then(invite => {
                if (invite.error || !invite.code) return message.channel.send(`⛔ | **${activity.name}** Başlatılamadı`);
          ...
          message.channel.send(aktivite) 
            ...
            .catch(e => {
                message.channel.send(`⛔ | **${activity.name}** Başlatılamadı`);
            })
    }
});
...
const emmmmbed = new Discord.MessageEmbed()
  ...
  .addField(`Nasıl Davet Ederim ?\n **\`${akardiyan.prefix}davet\` yazmanız yeterlidir**`) .setFooter(`Ovanakovic | Gelişmiş Türkçe Bot | 2021`)
  .setTimestamp();
...
client.on("message", async msg => {
  ...
  if (msg.content.toLowerCase() == `ovanakovic`)
...
client.on("message", async msg => {
  let prefix = (await db.fetch(`prefix.${msg.guild.id}`)) || akardiyan.prefix;
  if (msg.content == `<@!777498134828810272>`)
      ...
      try {
        if (!message.member.permissions.has("BAN_MEMBERS")) {
          ...
          return message.channel
            .send(`**${message.author} Bu Sunucuda Küfürü Engelliyorum!**`)
      ...
      try {
        if (!newMessage.member.permissions.has("BAN_MEMBERS")) {
          ...
          return newMessage.channel
            .send(`**${newMessage.author} Bu Sunucuda Küfürü Engelliyorum!**`)
            .then(message => message.delete(3000));
        }
      } catch (err) {
        console.log(err);
    ...
    if (
      ...
      try {
        if (!message.member.permissions.has("BAN_MEMBERS")) {
          ...
          return message.channel
            .send(`**${message.author} Bu Sunucuda Reklamı Engelliyorum!**`)
            .then(message => message.delete(3000));
        }
      } catch (err) {
        console.log(err);
    ...
    if (
      ...
      try {
        if (!newMessage.member.permissions.has("BAN_MEMBERS")) {
          ...
          return newMessage.channel
            .send(`**${newMessage.author} Bu Sunucuda Reklamı Engelliyorum!**`)
            .then(message => message.delete(3000));
        }
      } catch (err) {
        console.log(err);
